...
const QUIZ_REG_SEED = '我就是門 凡從我進來的 必然得救 並且出入得草吃';
...
begin
  ...
  s := UpperCase(MD5Print(MD5String('780927+' + edtMail.Text + '+790621')));
  for i := 1 to Length(s) do
    if not Odd(i) then sCode := sCode + s[i];

  sCode := Copy(sCode, 1, 8) + GetMidCode(QUIZ_REG_SEED + edtMail.Text + QUIZ_REG_SEED) + Copy(sCode, 9, 8);
  edtCode.Text := sCode;
end;

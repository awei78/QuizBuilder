...
SrcStrLen: TSrcLineLengths;

function yyleng : integer;         (* length of matched text *)
...
begin
  raise Exception.Create('Lex scanner error: ' + msg);
end;
...
function get_char: Char;
  ...
  procedure ProcessResultChar;
  begin
    if Result <> nl then
      Inc(yycolno)
    else
    begin
      SrcStrLen[yylineno mod Length(SrcStrLen)] := yycolno;
      yycolno := 0;
      Inc(yylineno);
    end;
  end;
...
begin
  if yyinput.Read(Result, 1) = 1 then
    ProcessResultChar
  else
  begin
    if Context.IncludeSourceStack.Count = 0 then
      Result := #0
    else
    begin
      repeat
        FreeAndNil(yyinput);
        with Context.IncludeSourceStack.Peek do
        begin
          yyinput := LexInput;
          yylineno := LexLineno;
          yycolno := LexColno;
          SrcStrLen := SrcLineLengths;
          yysrcfile := FileName;
        end;
        Context.IncludeSourceStack.Pop;
        if yyinput.Read(Result, 1) = 1 then
        begin
          ProcessResultChar;
          Exit;
        end;
      until Context.IncludeSourceStack.Count = 0;
      Result := #0;
    end;
  end;
end;
...
begin
  ...
  if yyinput.Position < 1 then
      Fatal('unget_char error: yyinput.Position = 0');
  yyinput.Seek(-1, soFromCurrent);
  ...
  begin
    Dec(yylineno);
    yycolno := SrcStrLen[yylineno mod Length(SrcStrLen)];
  end;
end;
...
begin
  ...
  begin
    C := #13; yyoutput.Write(C, 1);
    C := #10; yyoutput.Write(C, 1);
  end
  else
    yyoutput.Write(C, 1);
end;
